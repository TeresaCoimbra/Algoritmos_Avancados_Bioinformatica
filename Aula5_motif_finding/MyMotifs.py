...
class MyMotifs:
    ...
    def __init__(self, seqs, pseudo = False):
    ...
    def doCounts(self):
        if self.pseudo:
            self.counts = createMatOnes(len(self.alphabet), self.size)
        else:    
    ...
    def createPWM(self):
        ...
        if self.pseudo: 
            div = len(self.seqs)+len(self.alphabet)
        else:
            div = len(self.seqs)
        self.pwm = createMatZeros(len(self.alphabet), self.size)
        for i in range(len(self.alphabet)):
            for j in range(self.size):
                self.pwm[i][j] = float(self.counts[i][j]) / div
    ...
    def probAllPositions(self, seq):
        ...
        for k in range(len(seq)-self.size+1):
            res.append(self.probabSeq(seq[k:k+self.size]))
        return res
