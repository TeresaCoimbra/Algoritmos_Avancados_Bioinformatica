...
# Define MetabolicNetwork as a subclass of MyGraph
# Everything contained in MyGraph can be used by MetabolicNetwork

class MetabolicNetwork (MyGraph):
    
    def __init__(self, network_type = "metabolite-reaction", split_rev = False):
        # creates self.graph = {}:
        MyGraph.__init__(self, {})                           # creates graph of the MyGraph class by calling its constructor
        self.net_type = network_type 
        self.node_types = {}                                 
        if network_type == "metabolite-reaction":            # create lists of metabolits and reactions
            self.node_types["metabolite"] = []
            self.node_types["reaction"] = []
        self.split_rev =  split_rev                          
    
    def add_vertex_type(self, v, nodetype):
        '''Add node to bipartite graph and specify its type. Updates node list with node and nodetype.'''
        self.add_vertex(v)
        self.node_types[nodetype].append(v)
    
    def get_nodes_type(self, node_type):
        '''Get information on the node type.'''
