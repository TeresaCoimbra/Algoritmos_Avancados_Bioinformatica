...
## keys are vertices
## values of the dictionary represent the list of adjacent vertices of the
# key node
...
class MyGraph:
    ...
    def eulerian_cycle(self):
        ...
        res = [] 
        # temos de ir buscar um nó com arcos não cobertos, mas que já esteja naquele ciclo para fazer a ligação
        while edges_visit:   
            pair = edges_visit[0] #buscar o primeiro arco
            i = 1
            if res != []: # ir buscando outros pares e removendo
            ...
            while nxt != start: # creates cycle
                for suc in self.graph[nxt]:
                    if (nxt,suc) in edges_visit:
                        ...
                        cycle.append(nxt)
                        edges_visit.remove(pair)
            if not res: res = cycle # res mantém tudo
            else:
    ...
    def eulerian_path(self):
        ...
        if unb[0] is None or unb[1] is None: return None  # se não é semi-balanceado
        self.graph[unb[1]].append(unb[0])
        cycle = self.eulerian_cycle()
        for i in range(len(cycle)-1):
            if cycle[i] == unb[1] and cycle[i+1] ==  unb[0]: # fazer com que fiquem todos com o mesmo grau
                break
